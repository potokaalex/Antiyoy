...
namespace Client.ActionsHistory.Actions
{
  public class CreateUnitAction : IHistoryAction
  {
    ...
    private readonly bool? _oldUnitTurns;
    private readonly int _regionMoney;
    private SetRegionTypeRecoveryData _setRegionTypeRecoveryData;

    public CreateUnitAction(CellController cell, UnitType? oldUnitType, bool? oldUnitTurns, int regionMoney,
    ...
    public void Undo()
    {
      ...
      if(_oldUnitType.HasValue && _oldUnitTurns.HasValue)
        _unitsService.Create(_cell, _oldUnitType.Value, _oldUnitTurns.Value);
